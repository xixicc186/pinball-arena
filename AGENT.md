...
这一节不是讲“项目架构”，而是讲 Agent 真正接单时应该怎么做。

### 2.1 先判定任务类型

收到用户请求后，先把任务归到以下三类之一：

| 用户目标 | 应选模式 | 人数要求 | 应点击的按钮 |
|------|------|------|------|
| 录一局普通对战 | `自由混战` | 至少 2 名角色 | `开始出战` / `录制对局` |
| 录一届个人战 | `赛事玩法` + `个人战` | 恰好 8 名角色 | `开始赛事` / `录制整届赛事` |
| 录一届组队赛 | `赛事玩法` + `组队赛` | 恰好 16 名角色 | `开始赛事` / `录制整届赛事` |

如果人数不满足，不要硬开局；先补足或减少到正确人数。
...
- 只有被显式标记为“出战”的角色才会参赛
- 角色面板入口：
  - 自由混战：`#open-roster-button`
  - 赛事玩法：`#open-roster-button-tournament`
- roster 面板中的第二个 `.card-toggle` 才是“出战 / 不出战”开关
- `#select-all-button` 会把全部角色设为出战
- `#clear-roster-button` 会把全部角色设为不出战

### 2.3 Agent 选角原则

如果用户没有指定完整阵容，Agent 需要自己补齐名单。补齐时不要随机乱选，要综合以下因素：

- 玩法模式：自由混战、个人战、组队赛的目标不同
- 观赏性：优先激光、龙卷风、毒雾、分身、招魂、赌徒这类有明显演出的角色
- 平衡性：避免一边倒，保留反转空间
- 节奏：短视频需要较早出现爆点，长视频需要前中后期都有内容
- 角色相性：注意克制、联动、混乱度和镜头效果
- 成片时长：单局适合快速出片，赛事适合做完整赛程内容
...
- 录制结束后会出现两个浏览器确认框：
  - 第一个：是否导出这段视频
  - 第二个：是否同时转换为 MP4
- 如果用户明确要“视频放到项目根目录”，Agent 需要在下载完成后把文件移动到项目根目录
- 如果用户没指定格式，优先保留浏览器直接导出的文件；如果用户明确要 MP4，再执行第二个确认
...
在告诉用户“完成了”之前，至少检查以下内容：

- 模式是否正确
- 人数是否正确
- 角色名单是否和用户要求一致
- 视频文件是否真的下载成功
- 如果用户要求导出到项目根目录，文件是否已放到根目录
